import { AlertTriangle, ArrowLeft, CheckCircle2, FileWarning, Info, Printer } from 'lucide-react';
    ...
    <div className="py-12 print:py-0">
      ...
      <header className="grid gap-3 print:gap-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="secondary">lang: {a.language}</Badge>
        </div>
        <h1 className="text-balance text-3xl font-semibold tracking-tight md:text-4xl">
          Compliance audit report
        </h1>
        <p className="text-pretty text-muted-foreground">
          Generated by LexyFlow on {new Date(a.created_at).toLocaleString(params.locale)}.
          Risk score:{' '}
      ...
      <section className="mt-10 grid gap-4">
        ...
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No findings recorded — your document is in good shape against the selected frameworks.
          </p>
        ) : (
          rows.map((f) => (
      ...
      <section className="mt-12 hidden print:block text-xs text-muted-foreground">
        Generated by LexyFlow · lexyflow.com · audit id {a.id}
      </section>
    </div>
  );
}
